...
const formatActiveSponsorData = sponsorsData => {
  ...
  const processedData = sponsorsData
    .map(sponsor => ({
      ...
      !config.sponsorsToIgnore.includes(sponsor.name),
    );
  ...
  const processedData = sponsorsData
    .map(sponsor => ({
      ...
      website: sponsor.account.website
        ...
        : buildLinks(
          sponsor.account.socialLinks.find(
            link => link.type === 'WEBSITE',
          )?.url,
        ) ?? null,
      twitter:
...
const mainProcess = async () => {
  try {
    ...
    const activeSponsorsProcessedData = formatActiveSponsorData(activeSponsors.account.orders.nodes);
