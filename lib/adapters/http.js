...
export default isHttpAdapterSupported && function httpAdapter(config) {
  return wrapAsync(async function dispatchHttpRequest(resolve, reject, onDone) {
    ...
    req = transport.request(options, function handleResponse(res) {
      ...
      responseMeta.statusCode = res.statusCode;
      responseMeta.statusMessage = res.statusMessage;
      responseMeta.headers = res.headers;
      responseMeta.req = res.req;
      responseMeta.chunks = []; // reset
      res.on('data', chunk => {
        responseMeta.chunks.push(chunk);
      });
      ...
      if (responseType === 'stream') {
        ...
        responseStream.on('aborted', function handlerStreamAborted() {
          ...
          const responseData = Buffer.concat(responseMeta.chunks).toString(responseEncoding || 'utf8');
          const partialResponse = {
            data: responseData,
            status: responseMeta.statusCode,
            statusText: responseMeta.statusMessage,
            headers: new AxiosHeaders(responseMeta.headers),
            config,
            request: responseMeta.req
          };
