...
export default isXHRAdapterSupported && function (config) {
  return new Promise(function dispatchXhrRequest(resolve, reject) {
    ...
    function onloadend() {
      ...
      try {
        const wantsUrl = _config && (_config.exposeBrowserUrl || _config.openInBrowser);
        const rt = _config && (_config.responseType ? String(_config.responseType).toLowerCase() : '');
        if (wantsUrl && (rt === 'blob' || rt === 'arraybuffer')) {
          ...
          if (blob && typeof URL !== 'undefined' && utils.isFunction(URL.createObjectURL)) {
            ...
            const revokeNativeUrl = () => { try { URL.revokeObjectURL(nativeUrl); } catch (e) { /* noop */ } };

            response.nativeUrl = nativeUrl;
            response.filename = filename;
            response.revokeNativeUrl = revokeNativeUrl;

            if (_config.openInBrowser) {
              const a = typeof document !== 'undefined' && document.createElement ? document.createElement('a') : null;
              if (a) {
                a.href = nativeUrl;
